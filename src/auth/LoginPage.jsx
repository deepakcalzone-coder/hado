...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await fetch("http://103.181.108.248:5000/api/login", {
  method: "POST",
  headers: {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-slate-50 font-sans relative overflow-hidden px-4">
      {/* Subtle modern background glow blobs */}
      <div className="absolute top-0 left-1/3 w-96 h-96 bg-indigo-100 rounded-full mix-blend-multiply filter blur-3xl opacity-40 pointer-events-none" />
      <div className="absolute bottom-0 right-1/3 w-96 h-96 bg-violet-100 rounded-full mix-blend-multiply filter blur-3xl opacity-40 pointer-events-none" />
      ...
      <div className="w-full max-w-md bg-white p-8 sm:p-10 rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-100 relative z-10">
        ...
        <div className="flex flex-col items-center mb-8">
          <div className="h-9 w-9 rounded-xl bg-gradient-to-tr from-indigo-600 to-violet-500 flex items-center justify-center text-white font-black text-lg shadow-md shadow-indigo-200">
            Δ 
            
          </div>
          <span className="text-xl font-bold tracking-tight text-slate-900">HADO</span>
